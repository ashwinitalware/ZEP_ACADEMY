...
@Component({
  selector: 'app-forgotpassverify',
  templateUrl: './forgotpassverify.page.html',
  styleUrls: ['./forgotpassverify.page.scss'],
})
export class ForgotpassverifyPage implements OnInit {
  ...
  showOTPInput(f: NgForm){
    if (f.value.mobile !== '') {
      ...
      this.http
        .post(`${this.url.serverUrl}forget_verify_otp`, f.value)
        .subscribe(
          (res: any) => {
            console.log(res);
            {
  ...
  verify_submit(form: NgForm) {
    ...
    if (enteredOTP === this.otp) {
      ...
      this.router.navigate(['/forgotpass', { mobile: this.mobile }]);
    } else {
